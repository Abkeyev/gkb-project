    ...
    <div className="tab-content tab-2">
      <h3 className="title-subhead mb-16">
        ...
        <span className="number">
          {filterVoteRequests([11], main.clientData.user.id).length}
        </span>
      </h3>
      {filterVoteRequests([11], main.clientData.user.id).length === 0 ? (
        "Заявки отсутствуют."
      ) : (
        <table className="table req-table">
          ...
          <tbody>
            {filterVoteRequests([11], main.clientData.user.id).map(
              (r: Request) => (
