...
const AccessFormRejected = ({ request, filter }: AccessFormProps) => {
  const history = useHistory();
  return (
    <div className="tab-content tab-1">
      <h3 className="title-subhead mb-16">
        Отклоненные <span className="number">{filter([4]).length}</span>
      </h3>
      {filter([4]).length === 0 ? (
        "Заявки отсутствуют."
      ) : (
...
export default observer(AccessFormRejected);
